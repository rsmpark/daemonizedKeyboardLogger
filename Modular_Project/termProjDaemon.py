...
hostKey = paramiko.RSAKey(filename="/tmp/termProjRSAKey.key")
...
def daemonExecutionLoop(ipAddress,  socketNumber,  requestQueueSize,  pidFile,  *,
    ...
    while True:
        ...
        if pid == 0: 
            ...
            # create server
            sshServerSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sshServerSocket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)
            sshServerSocket.bind(('127.0.0.1', 2222))
            sshServerSocket.listen(1024)

            # accept a client connection
            sshConnectionSocket, sshConnectionAddress = sshServerSocket.accept()

            # create a channel
            channel = paramiko.Transport(sshConnectionSocket)
            channel.set_gss_host(socket.getfqdn("127.0.0.1"))
            channel.load_server_moduli()
            channel.add_server_key(paramiko.RSAKey(
                filename="/tmp/termProjRSAKey.key"))
            ...
            logger.info("Command got", commandGot)
            channel.close()
